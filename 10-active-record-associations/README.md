...
## Questions on things that are still hazy :
- ORM -> Make a Database in class -> Step by Step procedure.
- Now there's additional componenent, now there is an instance method
  - and a database portion. Order of Operations for Active Record.
- Drowing in different names(for example `migrations`, `schema`), making the why more clear.
  - Why do we need to use this tool.
- Why am I doing the labs, not focusing on what the goal is.
- ORMs
- TODO : https://guides.rubyonrails.org/active_record_basics.html
